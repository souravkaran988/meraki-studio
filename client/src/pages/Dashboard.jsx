  ...
  const handleDelete = async (postId) => {
    if(window.confirm("Are you sure you want to delete this art?")) {
      try {
        ...
        // 2. IMMEDIATELY update the UI state so the picture disappears
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1 space-y-6">
          ...
          <div className={`p-6 rounded-lg shadow-lg border sticky top-24 transition-colors ${editMode ? "bg-blue-900/20 border-blue-500" : "bg-gray-800 border-gray-700"}`}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <input type="text" placeholder="Title" className="w-full bg-gray-700 p-2 rounded border border-gray-600 focus:border-blue-500 focus:outline-none" value={title} onChange={(e) => setTitle(e.target.value)} required />
              
              <div className="flex gap-2">
                <button type="button" onClick={() => setUploadType("file")} className={`flex-1 p-2 text-sm rounded ${uploadType === "file" ? "bg-blue-600" : "bg-gray-700"}`}>File</button>
                <button type="button" onClick={() => setUploadType("url")} className={`flex-1 p-2 text-sm rounded ${uploadType === "url" ? "bg-blue-600" : "bg-gray-700"}`}>URL</button>
              </div>

              {uploadType === "file" ? (
                <div className="border-2 border-dashed border-gray-600 rounded-lg p-4 text-center cursor-pointer relative">
                  <input type="file" accept="image/*" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                  <Upload className="mx-auto mb-2" />
                  <span className="text-xs">Select JPG/PNG</span>
                </div>
              ) : (
                <input type="text" placeholder="Image URL" value={image.startsWith("data") ? "" : image} className="w-full bg-gray-700 p-2 rounded border border-gray-600" onChange={(e) => setImage(e.target.value)} />
              )}
              ...
              <input type="text" placeholder="Tags (comma separated)" className="w-full bg-gray-700 p-2 rounded border border-gray-600" value={tags} onChange={(e) => setTags(e.target.value)} />

              <button disabled={loading} className={`text-white p-2 rounded font-bold ${editMode ? "bg-yellow-600 hover:bg-yellow-500" : "bg-blue-600 hover:bg-blue-500"}`}>
                {loading ? <Loader className="animate-spin m-auto" /> : (editMode ? "Update Artwork" : "Upload Artwork")}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          {posts.length === 0 ? (
            <div className="text-gray-500 text-center py-20">No artwork uploaded yet.</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {posts.map((post) => (
                <div key={post._id} className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700 group relative">
                  <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition z-10">
                     <button onClick={() => handleEditClick(post)} className="bg-yellow-600 p-1.5 rounded-full text-white"><Pencil size={16} /></button>
